...
var resources = fs.readFileSync(path.join(__dirname, '../../api.json'));
resources = JSON.parse(resources.toString()).resources;
...
function getName(resourceName, actions) {
  var name = resourceName.toLowerCase();

  if (_.include(Object.keys(actions), 'List')) {
