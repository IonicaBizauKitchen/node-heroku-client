...
module.exports = function request(path, options, callback) {
  ...
  getCache(options.cacheKeyPostfix, path, function(cachedResponse) {
    ...
    var req = https.request(requestOptions, function(res) {
      ...
      } else if ((options.expectedResponse && (res.statusCode !== options.expectedResponse)) || (!options.expectedResponse && !res.statusCode.toString().match(/^2\d{2}$/))) {
        if (options.expectedResponse) {
          var message = 'Expected response ' + options.expectedResponse + ', got ' + res.statusCode
        } else {
